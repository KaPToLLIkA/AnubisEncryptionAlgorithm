...
namespace crypto {
    ...
    anubis::anubis()
    {
        auto key = generate_random_key(10);
        set_key(key);
    }
	...
	void anubis::set_key(std::vector<byte>& key)
	{
        ...
        for (int32_t r = 0; r <= R; ++r) 
        {
            ...
            rktmp[3] = T4[GET_BYTE(kappa[N - 1], 0)];

            for (int32_t t = N - 2; t >= 0; --t) 
            {
                rktmp[0] = T4[GET_BYTE(kappa[t], 24)] ^
                    (T5[GET_BYTE(rktmp[0], 24)] & 0xff000000) ^
                    (T5[GET_BYTE(rktmp[0], 16)] & 0x00ff0000) ^
                    (T5[GET_BYTE(rktmp[0], 8)]  & 0x0000ff00) ^
                    (T5[GET_BYTE(rktmp[0], 0)]  & 0x000000ff);

                rktmp[1] = T4[GET_BYTE(kappa[t], 16)] ^
                    (T5[GET_BYTE(rktmp[1], 24)] & 0xff000000) ^
                    (T5[GET_BYTE(rktmp[1], 16)] & 0x00ff0000) ^
                    (T5[GET_BYTE(rktmp[1], 8)]  & 0x0000ff00) ^
                    (T5[GET_BYTE(rktmp[1], 0)]  & 0x000000ff);

                rktmp[2] = T4[GET_BYTE(kappa[t], 8)] ^
                    (T5[GET_BYTE(rktmp[2], 24)] & 0xff000000) ^
                    (T5[GET_BYTE(rktmp[2], 16)] & 0x00ff0000) ^
                    (T5[GET_BYTE(rktmp[2], 8)]  & 0x0000ff00) ^
                    (T5[GET_BYTE(rktmp[2], 0)]  & 0x000000ff);

                rktmp[3] = T4[GET_BYTE(kappa[t], 0)] ^
                    (T5[GET_BYTE(rktmp[3], 24)] & 0xff000000) ^
                    (T5[GET_BYTE(rktmp[3], 16)] & 0x00ff0000) ^
                    (T5[GET_BYTE(rktmp[3], 8)]  & 0x0000ff00) ^
                    (T5[GET_BYTE(rktmp[3], 0)]  & 0x000000ff);
            }
            ...
            {
                inter[i] =
                    T0[GET_BYTE(kappa[i], 24)] ^
                    T1[GET_BYTE(kappa[(N + i - 1) % N], 16)] ^
                    T2[GET_BYTE(kappa[(N + i - 2) % N], 8)] ^
                    T3[GET_BYTE(kappa[(N + i - 3) % N], 0)];
            }
            ...
            memcpy(&kappa[1], &inter[1], (N - 1) * sizeof(uint32_t));
        ...
        for (int32_t r = 1; r < R; ++r) 
        {
           
            for (size_t i = 0; i < 4; ++i) 
            {
                uint32_t block = this->round_encrypt_key[R - r][i];


                this->round_decrypt_key[r][i] =
                    T0[T4[GET_BYTE(block, 24)] & 0xff] ^
                    T1[T4[GET_BYTE(block, 16)] & 0xff] ^
                    T2[T4[GET_BYTE(block, 8)] & 0xff] ^
                    T3[T4[GET_BYTE(block, 0)] & 0xff];
            }
        }
...
#ifdef _DEBUG
    std::vector<block32_t> anubis::get_round_encrypt_key()
    {
        return this->round_encrypt_key;
    }

    std::vector<block32_t> anubis::get_round_decrypt_key()
    {
        return this->round_decrypt_key;
    }
#endif // _DEBUG
